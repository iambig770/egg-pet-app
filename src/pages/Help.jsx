...
export default function Home() {
  ...
  const sellCharacter = async () => {
    ...
    setCharacters(prev => prev.filter(c => c.id !== current.id))
    setSelected(null)
  ...
  const getMaxFeed = (cur, energy) => {
    if (!cur || cur.stage >= 3) return 0
    const remainingMeals = STAGE_MEALS[cur.stage] - (cur.meals_fed || 0)
  ...
  const feedMeal = async () => {
    if (feeding) return
    if (!selected) return
    const cur = characters.find(c => c.id === selected.id)
    if (!cur) return
    if (cur.stage >= 3) return alert('이미 완성된 캐릭터입니다')
    if (userData.energy < 10) return alert('에너지가 부족합니다')

    const maxFeed = getMaxFeed(cur, userData.energy)
    const amount = Math.min(feedAmount, maxFeed)
    if (amount <= 0) return alert('에너지가 부족합니다')

    setFeeding(true)

    let meals = cur.meals_fed || 0
    let stage = cur.stage
    let totalEnergy = 0
    ...
    setCharacters(prev => prev.map(c =>
      c.id === cur.id ? { ...c, meals_fed: meals, stage } : c
    ))
    setUserData(prev => ({ ...prev, energy: prev.energy - totalEnergy }))

    await supabase.from('user_characters').update({ meals_fed: meals, stage }).eq('id', cur.id)
    ...
    <div style={{ padding: 20 }}>
      {userData.streak > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#FFF8E7', border: '1px solid #f39c12', borderRadius: 12, marginBottom: 16 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: '#f39c12' }}>{userData.streak}일 연속 달성 중</span>
        </div>
      )}
      ...
      {current ? (
        <div style={{ background: '#fff', border: '1px solid #ddd', borderRadius: 16, padding: 20, marginBottom: 16 }}>
          ...
          <div style={{ textAlign: 'center', fontSize: 13, color: '#888', marginBottom: 12 }}>{STAGE_LABEL[current.stage]}</div>
          {current.stage < 3 ? (
            <>
            ...
            <div style={{ textAlign: 'center', color: '#2F6B5A', fontWeight: 700, fontSize: 16 }}>🎉 완성!</div>
          )}
        </div>
      ) : (
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', marginBottom: 16 }}>
        ...
        <div onClick={buyEgg} style={{ width: 52, height: 52, borderRadius: 26, flexShrink: 0, border: '1.5px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, cursor: userData.coin >= 100 ? 'pointer' : 'default', color: userData.coin >= 100 ? '#2F6B5A' : '#ccc' }}>
          +
        </div>
      </div>
    </div>
